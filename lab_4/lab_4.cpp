...
int get_index(int x, int y, int z) { return x * N_Y * N_Z + y * N_Z + z; }
    ...
    for (int j = 1; j < N_Y - 1; j++) {
      for (int k = 1; k < N_Z - 1; k++) {
        f_i = (prev_func[get_index(i + 1, j, k)] +
               prev_func[get_index(i - 1, j, k)]) /
              H_X_2;
        f_j = (prev_func[get_index(i, j + 1, k)] +
               prev_func[get_index(i, j - 1, k)]) /
              H_Y_2;
        f_k = (prev_func[get_index(i, j, k + 1)] +
               prev_func[get_index(i, j, k - 1)]) /
              H_Z_2;
        ...
        tmp_max_diff =
            fabs(curr_func[get_index(i, j, k)] - prev_func[get_index(i, j, k)]);
...
double calc_border(const double *prev_func, double *curr_func,
                   ...
                   int layer_height, int offset, int proc_rank,
                   int proc_count) {
  double f_i = 0.0;
  double f_j = 0.0;
  double f_k = 0.0;
    ...
    for (int k = 1; k < N_Z - 1; k++) {
      if (proc_rank != 0) {
        f_i = (prev_func[get_index(1, j, k)] +
               up_border_layer[get_index(0, j, k)]) /
              H_X_2;
        f_j = (prev_func[get_index(0, j + 1, k)] +
               prev_func[get_index(0, j - 1, k)]) /
              H_Y_2;
        f_k = (prev_func[get_index(0, j, k + 1)] +
               prev_func[get_index(0, j, k - 1)]) /
              H_Z_2;
        ...
        tmp_max_diff =
            fabs(curr_func[get_index(0, j, k)] - prev_func[get_index(0, j, k)]);
        if (tmp_max_diff > max_diff) {
          max_diff = tmp_max_diff;
        }
      }
      ...
      if (proc_rank != proc_count - 1) {
        f_i = (prev_func[get_index(layer_height - 2, j, k)] +
               down_border_layer[get_index(0, j, k)]) /
              H_X_2;
        f_j = (prev_func[get_index(layer_height - 1, j + 1, k)] +
               prev_func[get_index(layer_height - 1, j - 1, k)]) /
              H_Y_2;
        f_k = (prev_func[get_index(layer_height - 1, j, k + 1)] +
               prev_func[get_index(layer_height - 1, j, k - 1)]) /
              H_Z_2;
...
int main(int argc, char **argv) {
  ...
  prev_func =
      (double *)malloc(sizeof(double) * layer_heights[proc_rank] * N_Y * N_Z);
  curr_func =
      (double *)malloc(sizeof(double) * layer_heights[proc_rank] * N_Y * N_Z);
  init_layers(prev_func, curr_func, layer_heights[proc_rank],
              offsets[proc_rank]);
  ...
  do {
    ...
    if (proc_rank != 0) {
      ...
      MPI_Isend(prev_up_border, N_Y * N_Z, MPI_DOUBLE, proc_rank - 1, proc_rank,
    ...
    MPI_Allreduce(&prev_proc_max_diff, &max_diff, 1, MPI_DOUBLE, MPI_MAX,
                  MPI_COMM_WORLD);
  ...
  max_diff =
      calc_max_diff(curr_func, layer_heights[proc_rank], offsets[proc_rank]);
